...
export function extractJSDocInfo(symbol: ts.Symbol | undefined, checker: ts.TypeChecker): JSDocInfo {
...
export function extractTypeIR(type: ts.Type, checker: ts.TypeChecker, cache = new Map<ts.Type, TypeIR>()): TypeIR {
    ...
    const jsDocInfo = typeName ? extractJSDocInfo(symbol, checker) : { constraints: [], examples: [], meta: {} };
    ...
    if (type.flags & ts.TypeFlags.BooleanLiteral) {
        const isTrue =
            "intrinsicName" in type && (type as ts.Type & { intrinsicName?: string }).intrinsicName === "true";
        const res: TypeIR = {
            id: nextId(cache),
            kind: "literal",
            value: isTrue,
    ...
    function findDiscriminatorProperty(types: TypeIR[]): { propertyName: string } | undefined {
        ...
        const candidateProps = firstObj.properties.filter((p) => !p.optional && p.type.kind === "literal");
    ...
    for (const propSymbol of properties) {
        ...
        const propType = propDecl ? checker.getTypeOfSymbolAtLocation(propSymbol, propDecl) : checker.getAnyType();
