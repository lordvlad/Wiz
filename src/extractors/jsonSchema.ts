...
export function jsonSchemaToIR(
    ...
    return {
      ...
      properties: Object.entries(propsObj).map(([propName, propRaw]) => ({
        ...
        type: jsonSchemaToIR(
          propRaw,
          ctx,
          `${pointer}/properties/${token(propName)}`
        ),
        ...
        description:
          isObject(propRaw) && typeof propRaw.description === "string"
            ? propRaw.description
            : undefined,
      })),
    };
  ...
  if (Array.isArray(raw.oneOf) || Array.isArray(raw.anyOf)) {
    ...
    return {
      id: nextId(),
      kind: "union",
      types: members.map((member, index) =>
        jsonSchemaToIR(member, ctx, `${pointer}/${key}/${index}`)
      ),
    };
  }
