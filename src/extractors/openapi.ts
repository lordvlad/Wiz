...
function reportUnsupported(schema: JsonObject, ctx: Ctx, pointer: string): void {
    for (const keyword of UNSUPPORTED_SCHEMA_KEYWORDS) {
        ...
        diagnose(ctx, `${pointer}/${keyword}`, keyword, `dropped unrepresentable schema keyword '${keyword}'`);
    }
...
const NUMERIC_KEYWORDS = ["minimum", "maximum", "exclusiveMinimum", "exclusiveMaximum", "multipleOf"] as const;
...
function checkMismatchedConstraints(raw: JsonObject, types: string[] | undefined, ctx: Ctx, pointer: string): void {
...
function schemaToIR(raw: unknown, ctx: Ctx, pointer: string): TypeIR {
    ...
    const consumedFormat =
        Boolean(nonNull?.includes("string")) && typeof raw.format === "string" && raw.format in PRIMITIVE_FORMATS;
    ...
    return {
        ...
        ...annotated,
        kind: "union",
        types: [coreToIR(raw, ctx, pointer, nonNull, {}), { id: nextId(ctx), kind: "primitive", type: "null" }],
    };
}
...
function coreToIR(
    ...
    if (Array.isArray(raw.allOf)) {
        const types = raw.allOf.map((member, index) => schemaToIR(member, ctx, `${pointer}/allOf/${index}`));
    ...
    if (variants) {
        ...
        const propertyName =
            variants.key === "oneOf" && isObject(discriminator) && typeof discriminator.propertyName === "string"
...
function arrayToIR(raw: JsonObject, ctx: Ctx, pointer: string, base: { id: string } & Annotated): TypeIR {
...
function objectToIR(raw: JsonObject, ctx: Ctx, pointer: string, base: { id: string } & Annotated): TypeIR {
    const required = Array.isArray(raw.required) ? raw.required.filter((n): n is string => typeof n === "string") : [];
...
const COMPONENT_SECTIONS = ["parameters", "headers", "requestBodies", "responses", "securitySchemes"] as const;
 ...
 */
function componentDefinition(ctx: Ctx, section: ComponentSection, name: string): JsonObject | undefined {
...
function parameterToIR(
    ...
    if (raw.schema === undefined && raw.content !== undefined) {
        diagnose(ctx, `${pointer}/content`, "content", "skipped parameter declared with `content` instead of `schema`");
        return undefined;
    }
...
function bodiesFor(content: unknown, ctx: Ctx, pointer: string): ServiceMethodBodyIR[] {
    ...
    return Object.entries(content).map(([mimetype, media]) => ({
        mimetype,
        content: schemaToIR(isObject(media) ? media.schema : undefined, ctx, `${pointer}/${token(mimetype)}/schema`),
    }));
}
...
function resolveHeader(name: string, raw: JsonObject, ctx: Ctx, pointer: string): ParameterIR | undefined {
    if (typeof raw.$ref === "string") {
...
function responseToIR(raw: JsonObject, ctx: Ctx, pointer: string, status: number | "default"): HttpResponseIR {
...
function collectParameters(raw: unknown, ctx: Ctx, pointer: string, into: Map<string, ParameterIR>): void {
...
function requestToIR(
    ...
    if (isObject(body)) {
        if (typeof body.$ref === "string") {
            ...
            if (!registered) {
                throw new Error(`[wiz] unresolved $ref '${body.$ref}' at ${pointer}/requestBody; no such component`);
            }
...
function responsesToIR(operation: JsonObject, ctx: Ctx, pointer: string): ServiceMethodResponseIR[] {
...
function detectVersion(document: JsonObject): "3.0" | "3.1" {
    if (document.swagger !== undefined) {
        throw new Error(`[wiz] unsupported OpenAPI version '${String(document.swagger)}'; 3.0 and 3.1 are supported`);
    }
    ...
    throw new Error(`[wiz] unsupported OpenAPI version '${String(declared)}'; 3.0 and 3.1 are supported`);
}
