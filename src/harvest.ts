 ...
 */
function warnUndocumentable(logger: WizLogger, message: string, node: ts.Node, sourceFile: ts.SourceFile): void {
...
export function collectOperations(
    ...
    for (const element of arg.elements) {
        ...
        const [pathParams, queryParams, response, requestBody] = (element.typeArguments ?? []).map((typeNode) =>
                ...
                if (fnName === "openapiSchema" || fnName === "openapiDocument") {
                    ...
                    if (typeNode) {
                        ...
                        if (methods.length > 0) {
                            ...
                            fragments.push(runGeneratedDocument(code, (baseValue as Record<string, unknown>) ?? {}));
                        }
                    }
                }
            }
...
export function extractMethodsFromObjectType(type: ts.Type, checker: ts.TypeChecker): ObjectTypeMethod[] {
...
function resolveTypeByName(
    ...
    for (const scope of scopes) {
        sym = checker.getSymbolsInScope(scope, meaning).find((candidate) => candidate.name === typeName);
        if (sym) {
            break;
        }
    }
...
function parseAsyncApiMethodFromSignature(
    ...
    if (listener) {
        ...
        msgType = handled
            ? checker.getTypeOfSymbolAtLocation(handled, handled.valueDeclaration ?? handled.declarations?.[0]!)
    ...
    for (const elemType of typeArgs) {
        ...
        if (signatures.length > 0) {
            ...
            for (const m of objectMethods) {
                ...
                // namespaced again.
                const explicitName = (typeof rawName === "string" && rawName ? rawName : undefined) ?? taggedName;
