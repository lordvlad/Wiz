import { INTEGER_FORMATS, SAFE_INTEGER, type Annotated, type Constraint, type ConstraintKind } from "./types.ts";
 ...
 * author never wrote.
 */
export function integerFormatRange(format: unknown): { minimum?: number; maximum?: number } | undefined {
 ...
 */
export function annotationsToKeywords(node: Annotated, version: OpenApiVersion): Record<string, unknown> {
    const keywords: Record<string, unknown> = {};
 ...
 */
export function vendorExtensions(source: Record<string, unknown>): Record<string, unknown> | undefined {
