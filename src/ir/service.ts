...
export type HttpMethodName = "GET" | "POST" | "PUT" | "PATCH" | "DELETE" | "HEAD" | "OPTIONS" | "TRACE";
...
export function normalizeServiceMethod(method: ServiceMethodIR): unknown {
    ...
    if (isMcpMethod(method)) {
        const mcpResp = method.responses.find((candidate): candidate is McpResponseIR => candidate.protocol === "mcp");
        return {
            a: method.address,
            t: method.title ?? null,
            de: method.description ?? null,
    ...
    if (!isHttpMethod(method)) {
        throw new Error(`[wiz] cannot key a '${method.protocol}' service method: no normal form for it`);
    }
