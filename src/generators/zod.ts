...
function baseFor(ir: TypeIR, needs: Needs): string {
  switch (ir.kind) {
    ...
    case "enum": {
      if (ir.members.length === 0) return "z.never()";
      ...
      if (ir.members.every((member) => typeof member.value === "string")) {
        return `z.enum([${ir.members
          .map((member) => JSON.stringify(member.value))
          .join(", ")}])`;
      }
      return `z.union([${ir.members
        .map((member) => `z.literal(${literalValue(member.value)})`)
        .join(", ")}])`;
    }
    ...
    case "array":
      return constrain(
        `z.array(${zodFor(ir.element, needs)})`,
        ir.constraints,
        "array",
        needs
      );
    ...
    case "tuple": {
      ...
      if (ir.rest) return inexpressible("a tuple with a rest element");
      if (ir.elements.some((element) => element.optional)) {
        return inexpressible("a tuple with optional elements");
      }
      return `z.tuple([${ir.elements
        .map((element) => zodFor(element.type, needs))
        .join(", ")}])`;
    }
    ...
    case "union": {
      ...
      if (ir.types.length === 0) return "z.never()";
      if (ir.types.length === 1) return zodFor(ir.types[0]!, needs);
      return `z.union([${ir.types
        .map((member) => zodFor(member, needs))
        .join(", ")}])`;
    }

    case "intersection": {
      if (ir.types.length === 0) return "z.unknown()";
      return ir.types
        .map((member) => zodFor(member, needs))
    ...
    case "record":
      return `z.record(${zodFor(ir.keyType, needs)}, ${zodFor(
        ir.valueType,
        needs
      )})`;
...
function objectFor(
  ...
  const entries = ir.properties.map((prop) => {
    ...
    if (prop.optional) value += ".optional()";
    return `${propertyKey(prop.name)}: ${value}`;
  });

  const shape = `z.object({ ${entries.join(", ")} })`;

  if (ir.additionalProperties === undefined) return shape;
  if (ir.additionalProperties === true) return `${shape}.passthrough()`;
  if (ir.additionalProperties === false) return `${shape}.strict()`;
...
function constrain(
  ...
  if (!family || !constraints || constraints.length === 0) return expr;
  ...
  for (const c of constraints) {
    switch (c.kind) {
      ...
      case "exclusiveMaximum":
        ...
        break;
      case "multipleOf":
        if (family === "number") out += `.multipleOf(${Number(c.value)})`;
        break;
      case "minLength":
        if (family === "string") out += `.min(${Number(c.value)})`;
        break;
      case "maxLength":
        if (family === "string") out += `.max(${Number(c.value)})`;
        break;
      case "pattern":
        if (family === "string") {
        ...
        break;
      case "minItems":
        if (family === "array") out += `.min(${Number(c.value)})`;
        break;
      case "maxItems":
        if (family === "array") out += `.max(${Number(c.value)})`;
        break;
      case "uniqueItems":
...
function formatFor(value: unknown, family: Family): string {
  ...
  if (family === "bigint") return `.min(${range.min}n).max(${range.max}n)`;

  if (family === "number") {
    // A bound a `number` cannot hold exactly is left out rather than rounded:
    // rounded, it would reject values the width allows.
    let out = ".int()";
    if (range.min >= -SAFE_INTEGER) out += `.min(${Number(range.min)})`;
    if (range.max <= SAFE_INTEGER) out += `.max(${Number(range.max)})`;
    return out;
  }
...
function scalar(value: unknown, family: "number" | "bigint"): string {
  if (family === "number") return String(Number(value));
  const asBigInt =
