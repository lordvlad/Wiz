...
function collectMethodVariants(
  ...
  const effectiveResponses =
    responseBodies.length > 0
      ? responseBodies
      : [{ mimetype: 'application/json', content: undefined }];
...
function generateJakartaClientSource(
  ...
  for (const method of service.methods) {
    ...
    for (const v of variants) {
      ...
      lines.push(
        `    WebTarget resource = this.target.path(${JSON.stringify(http.address.path)});`
      );
...
function generateMicroProfileClientSource(
  ...
  for (const method of service.methods) {
    ...
    for (const v of variants) {
      ...
      for (const p of pathParams) {
        methodParams.push(
          `@PathParam("${p.name}") ${toJavaType(p.type, typeNameMap)} ${toCamelCase(p.name)}`
        );
      }
      for (const p of queryParams) {
        methodParams.push(
          `@QueryParam("${p.name}") ${toJavaType(p.type, typeNameMap)} ${toCamelCase(p.name)}`
        );
      }
      for (const p of headerParams) {
        methodParams.push(
...
export function generateJavaFiles(
  ...
  for (const [name, ir] of types) {
    ...
    if (ir.kind === 'enum') {
      ...
      files[fileName] = generateClassSource(
        className,
        ir,
        options,
        typeNameMap,
