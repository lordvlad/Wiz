...
import type { TypeIR, PropertyIR, EnumMemberIR } from "../ir/types.ts";
...
export function mimetypeToSuffix(mimetype: string): string {
    ...
    if (norm.includes("yaml")) {
        return "AsYaml";
    }
    if (norm.includes("csv")) {
        return "AsCsv";
    }
    if (norm.includes("octet-stream")) {
        return "AsOctetStream";
    }
    if (norm.includes("form-data")) {
        return "AsFormData";
    }
    if (norm.includes("x-www-form-urlencoded")) {
        return "AsUrlEncoded";
    }
 ...
 */
function collectMethodVariants(method: HttpServiceMethodIR, typeNameMap: Map<string, string>): HttpMethodVariant[] {
    ...
    const isSuccess = (r: HttpResponseIR) => typeof r.status === "number" && r.status >= 200 && r.status < 300;
    ...
    if (!hasMultipleResponses && !hasMultipleRequests) {
        ...
        const returnType = respBody?.content ? toJavaType(respBody.content, typeNameMap) : "void";
    ...
    const effectiveResponses =
        responseBodies.length > 0 ? responseBodies : [{ mimetype: "application/json", content: undefined }];
...
function generateJakartaClientSource(
    ...
    const clientName =
        options.clientName ?? (effectiveServiceName ? `${toPascalCase(effectiveServiceName)}Client` : "ApiClient");
    const lines: string[] = [];
    ...
    for (const method of service.methods) {
        ...
        for (const v of variants) {
            ...
            const pathParams = (http.request.parameters ?? []).filter((p) => p.in === "path");
            const queryParams = (http.request.parameters ?? []).filter((p) => p.in === "query");
            const headerParams = (http.request.parameters ?? []).filter((p) => p.in === "header");
            ...
            if (http.description || http.summary) {
                ...
                if (http.description) {
                    lines.push(`   * ${http.description}`);
                }
                if (v.responseMimetype) {
                    lines.push(`   * Accepts: ${v.responseMimetype}`);
                }
                lines.push("   */");
            }
...
function generateMicroProfileClientSource(
    ...
    for (const method of service.methods) {
        ...
        for (const v of variants) {
            ...
            for (const p of headerParams) {
                methodParams.push(
                    `@HeaderParam("${p.name}") ${toJavaType(p.type, typeNameMap)} ${toCamelCase(p.name)}`,
                );
            }
            if (bodyParam) {
            ...
            if (http.description || http.summary) {
                ...
                if (v.responseMimetype) {
                    lines.push(`   * Produces: ${v.responseMimetype}`);
                }
                lines.push("   */");
            }
