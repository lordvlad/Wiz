...
import { annotationsToKeywords, constraintsToKeywords } from "../openapiDialect.ts";
import { collectNamedTypes, isUserNamedType, type Annotated, type Constraint, type TypeIR } from "../types.ts";
...
function applyAnnotations(schema: Record<string, unknown>, node: Annotated, version: "3.0" | "3.1") {
    Object.assign(schema, annotationsToKeywords(node, version));
}

export function irToOpenApiSchema(ir: TypeIR, version: "3.0" | "3.1", isRoot = false): Record<string, unknown> {
    ...
    switch (ir.kind) {
        ...
        case "object": {
            ...
            if (ir.additionalProperties !== undefined) {
                if (typeof ir.additionalProperties === "boolean") {
                    ...
                    schema.additionalProperties = irToOpenApiSchema(ir.additionalProperties, version, false);
                }
            }
            break;
        }
        ...
        case "tuple": {
            ...
            if (version === "3.1") {
                if (ir.elements.length > 0) {
                    schema.prefixItems = ir.elements.map((e) => irToOpenApiSchema(e.type, version, false));
                }
                if (ir.rest) {
                ...
                if (ir.elements.length > 0) {
                    schema.items = ir.elements.map((e) => irToOpenApiSchema(e.type, version, false));
                }
            }
            break;
        }
        ...
        case "union": {
            ...
            const nonNull = present.filter((t) => !(t.kind === "primitive" && t.type === "null"));
            const nullable = present.length > nonNull.length;

            if (nonNull.length === 0) {
                return version === "3.0" ? { ...schema, nullable: true } : { ...schema, type: "null" };
            }
            ...
            if (nonNull.length === 1) {
                ...
                if (version === "3.0") {
                    return { ...schema, ...base, nullable: true };
                }
        ...
        case "record": {
            schema.type = "object";
            schema.additionalProperties = irToOpenApiSchema(ir.valueType, version, false);
            break;
        }
...
function parametersFor(parameters: ParameterIR[] | undefined, version: "3.0" | "3.1"): Record<string, unknown>[] {
    if (!parameters) {
        return [];
    }
    const emitted: Record<string, unknown>[] = [];
    for (const p of parameters) {
        if (isAbsentIR(p.type)) {
            continue;
        }
 ...
 */
function operationSource(method: HttpServiceMethodIR, version: "3.0" | "3.1"): string {
    ...
    if (method.operationId ?? methodName) {
        operation.operationId = method.operationId ?? methodName;
    }
    if (pkg) {
        operation["x-package"] = pkg;
    }
    if (svc) {
    ...
    for (const response of method.responses) {
        ...
        if (!existing) {
            responses[key] = {
                description: response.description ?? (content ? "Successful response" : "No content"),
    ...
    return method.overrides ? `{ ...${generated}, ...(${method.overrides}) }` : generated;
}
...
function pathsSource(service: ServiceIR, version: "3.0" | "3.1"): string {
    ...
    for (const method of service.methods) {
        ...
        entries.push([method.address.method.toLowerCase(), operationSource(method, version)]);
        byPath.set(method.address.path, entries);
    }

    const pathEntries = [...byPath.entries()].map(([path, methods]) => {
        const methodEntries = methods.map(([method, src]) => `    ${JSON.stringify(method)}: ${src}`).join(",\n");
...
function typesReferencedBy(method: HttpServiceMethodIR): TypeIR[] {
    ...
    for (const p of request.parameters ?? []) {
        referenced.push(p.type);
    }
    for (const body of request.body ?? []) {
        referenced.push(body.content);
    }
    for (const response of method.responses) {
...
export function generateOpenApiSchemaCode(
    ...
    for (const method of service.methods) {
        ...
        for (const ir of typesReferencedBy(method)) {
            collect(ir);
        }
    }
