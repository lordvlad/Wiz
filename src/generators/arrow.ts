...
import { declaredFormat, flattenObjectProperties, type Annotated, type TypeIR } from "../types.ts";
...
function arrowTypeOf(field: ArrowField, A: ArrowModule): unknown {
    ...
    const children = field.children.map((child) => new A.Field(child.name, arrowTypeOf(child, A), child.nullable));
...
export function arrowSchemaMessage(fields: ArrowField[]): Uint8Array {
    const A = loadArrow();
    const schema = new A.Schema(fields.map((field) => new A.Field(field.name, arrowTypeOf(field, A), field.nullable)));
