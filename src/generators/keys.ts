...
function extractDeepKeys(
    ...
    if (ir.kind === "ref" && declaredTypes) {
        ...
        if (target) {
            return extractDeepKeys(target, declaredTypes, prefix, depth, maxDepth, new Set([...visited, ir.targetId]));
        }
    }
    ...
    for (const prop of ir.properties) {
        ...
        const childKeys = extractDeepKeys(prop.type, declaredTypes, keyPath, depth + 1, maxDepth, nextVisited);
