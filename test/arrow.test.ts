...
const codecFor = (source: string, name = "M") => evalModule<Codec>(generateArrowCode(getIRForSource(source, name)));
...
describe("apache-arrow reads what wiz writes", () => {
    ...
    `);
        ...
        expect(table.toArray().map((r: any) => [r.id, r.name])).toEqual(rows.map((r) => [r.id, r.name]));
    });
