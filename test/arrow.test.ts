...
describe("the schema describes the columns", () => {
  ...
  test("nested shapes describe as list, struct and map", () => {
    ...
    expect(byName.counts.children[0].children.map((c: any) => c.name)).toEqual([
      "key",
      "value",
    ]);
  });
...
describe("apache-arrow reads what wiz writes", () => {
  test("every scalar column, at the widths @format selects", () => {
    ...
    const rows = [
      { id: 1, name: "a", ok: true, ratio: 1.5, approx: 0.5, big: 9007199254740993n, at: new Date(1000), blob: new Uint8Array([1, 2]) },
      { id: -2147483648, name: "bb", ok: false, ratio: -0.25, approx: -1.5, big: -9007199254740993n, at: new Date(2000), blob: new Uint8Array([3]) },
      { id: 2147483647, name: "", ok: true, ratio: 0, approx: 0, big: 0n, at: new Date(0), blob: new Uint8Array([]) },
    ];

    const table = A.tableFromIPC(writeRows(codec, rows));
    expect(table.numRows).toBe(3);
    expect(table.schema.fields.map((f) => f.name)).toEqual([
      "id", "name", "ok", "ratio", "approx", "big", "at", "blob",
    ]);
    ...
    expect(seen).toEqual(rows.map((r) => ({
      id: r.id,
      name: r.name,
      ok: r.ok,
      ratio: r.ratio,
