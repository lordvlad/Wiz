...
const emit = (validate: unknown) => generate(api, tsClientGenerator, { validate } as never, silentLogger);
...
describe("the --validate flag", () => {
    ...
    const targetsFor = async (...flags: string[]): Promise<string[]> => {
        ...
        const found = [...source.matchAll(/ClientValidationError\("([a-z]+)"/g)].map((match) => match[1]!);
        return [...new Set(found)].sort();
    };

    test("without a value it validates everything", async () => {
        expect(await targetsFor("--validate")).toEqual(["body", "headers", "path", "query", "response"]);
    });
    ...
    test("the bare flag does not consume the input path", async () => {
        ...
        expect(await runGenerate(["-g", "tsClient", "--validate", documentPath, "-o", outdir])).toBe(0);
        expect(await Bun.file(join(outdir, "api.ts")).exists()).toBe(true);
    });

    test("a misspelled target is named, not silently ignored", async () => {
        expect(runGenerate(["-g", "tsClient", documentPath, "--validate=respones"])).resolves.toBe(1);
        expect(runGenerate(["-g", "tsClient", documentPath, "--validate", "path,respones"])).resolves.toBe(1);
    });
});
...
describe("emitted code compiles and runs", () => {
    ...
    test("a request the document allows is sent", async () => {
        ...
        expect(await client.getPet({ petId: "abcdef" }, { "x-token": "t" })).toEqual(valid);
    });
    ...
    test("a path parameter breaking its constraint is rejected", async () => {
        const client = await load(valid);
        const error = await rejection(() => client.getPet({ petId: "abc" }, { "x-token": "t" }));
    ...
    test("a query parameter breaking its constraint is rejected", async () => {
        const client = await load(valid);
        const error = await rejection(() => client.getPet({ petId: "abcdef" }, { "x-token": "t" }, { limit: 0 }));
    ...
    test("a request body breaking its constraint is rejected", async () => {
        const client = await load(valid);
        const error = await rejection(() => client.createPet({ petId: "p1" }, { id: "1", name: "ab" }));
    ...
    test("a response the server got wrong is rejected", async () => {
        ...
        const error = await rejection(() => client.getPet({ petId: "abcdef" }, { "x-token": "t" }));
