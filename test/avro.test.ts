...
function schemaFor(source: string, typeName: string): any {
    ...
    const mod = evalModule<{ avroSchema: (o?: any) => string }>(generateAvroSchemaCode([{ name: typeName, ir }]));
...
describe("avro schema", () => {
    ...
    test("indent is honoured, and the result parses as JSON", () => {
        ...
        const mod = evalModule<{ avroSchema: (o?: any) => string }>(generateAvroSchemaCode([{ name: "User", ir }]));
...
describe("avro binary codec", () => {
    ...
    test("long keeps full 64-bit precision", () => {
        ...
        for (const v of [9007199254740993n, -9007199254740993n, 9223372036854775807n, -9223372036854775808n]) {
