...
const avro = (src: string, name = "M") => evalModule<Avro>(generateAvroCode(getIRForSource(src, name)));
...
describe("Uint8Array and Date are opaque scalars", () => {
  ...
  `;
    ...
    test("the validator checks the real runtime classes", () => {
        const mod = evalModule<{ is: (v: unknown) => boolean }>(generateValidatorCode(getIRForSource(src, "M")));
...
describe("extra integer widths", () => {
  ...
  `;
    ...
    test("unsigned, zigzag and fixed widths reach the .proto output", () => {
        for (const format of ["uint32", "uint64", "sint32", "sint64", "fixed32", "sfixed32", "fixed64", "sfixed64"]) {
            const text = evalModule<{ protobufSchema: (o?: any) => string }>(
