...
describe("emitted files", () => {
    ...
    test("each operation takes exactly the slots it declares", () => {
        ...
        expect(source).toContain("export type ListPetsOptions = { query?: { limit?: number } };");
        ...
        expect(source).toContain("createPet(body: NewPet, callOptions?: HttpCallOptions): Promise<CreatePetResult>;");
        ...
        expect(source).toContain("export type DeletePetOptions = { path: { petId: string } };");
        expect(source).toContain("export type DeletePetResult = void;");
        expect(source).toContain("deletePet(path: { petId: string }, callOptions?: HttpCallOptions): Promise<void>;");
    });
    ...
    test("operations are reachable as a client and as module-level functions", () => {
        ...
        expect(source).toContain("export function createClient(overrides: Partial<ClientConfig> = {}): Client {");
...
describe("emitted code compiles and runs", () => {
    ...
    test("typechecks under strict TypeScript", () => {
        ...
        const diagnostics = [...program.getSyntacticDiagnostics(), ...program.getSemanticDiagnostics()].map(
    ...
    type InterceptorLike = (call: Sent, next: (call: Sent) => Promise<ResultLike>) => Promise<ResultLike>;
    ...
    test("a path parameter is encoded into the URL and the response is parsed", async () => {
        ...
        const pet = await client.getPetById({ petId: "p 1" }, { "x-trace-id": "t1" });
    ...
    test("a failure throws with the status and the parsed body", async () => {
        ...
        const failure = await client.getPetById({ petId: "p1" }).catch((error: unknown) => error);
        if (!(failure instanceof client.ApiError)) {
    ...
    test("createClient keeps two configurations apart", async () => {
        ...
        const tenantA = client.createClient({
            ...
            interceptors: {
                http: [(call, next) => next({ ...call, headers: { ...call.headers, authorization: "Bearer a" } })],
            },
        });
    ...
    test("the chain nests outermost first, and may call next twice", async () => {
        ...
        const retrying = client.createClient({
            ...
            transport: async () => {
                attempts += 1;
                return attempts === 1 ? json({ error: "busy" }, 503) : json({ id: "p1", name: "Rex" });
            },
            interceptors: {
                http: [
                    async (call, next) => {
    ...
    test("custom HttpTransport overrides fetch symmetrically to gRPC", async () => {
        ...
        const _pet = await customClient.getPetById({ petId: "t1" });
    ...
    test("a call can be cancelled through its signal, and a timeout aborts it", async () => {
        ...
        const inFlight = controlled.getPetById({ petId: "x" }, undefined, {
            signal: controller.signal,
        });
        const _cancelled = await inFlight.catch((error: unknown) => error);
        ...
        const defaulted = await byDefault.deletePet({ petId: "x" }).catch((error: unknown) => error);
...
describe("mediaTypes option support in tsClient", () => {
    test("emits client with YAML, JSONL, Erlang, XML support when mediaTypes is set", () => {
        ...
        const files = generate(ir, tsClientGenerator, { mediaTypes: ["yaml", "jsonl", "erlangText"] }, silentLogger);
    ...
    test("emits client with all media types when mediaTypes is 'all'", () => {
        ...
        const files = generate(ir, tsClientGenerator, { mediaTypes: "all" }, silentLogger);
