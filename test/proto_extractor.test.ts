...
const propertyOf = (ir: TypeIR | undefined, name: string) => {
  if (ir?.kind !== "object") throw new Error("expected an object type");
  const property = ir.properties.find((candidate) => candidate.name === name);
  if (!property) throw new Error(`no property '${name}'`);
  return property;
};
...
describe("proto scalars", () => {
  ...
  test("every scalar maps to a JS type plus the width it travels as", () => {
    const shape = (field: string) => {
      ...
      if (type.kind !== "primitive") throw new Error(`${field} is not primitive`);
      return [
        type.type,
        type.constraints?.find((c) => c.kind === "format")?.value ?? null,
      ];
    };
