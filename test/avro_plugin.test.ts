...
import { avroSchema, decodeAvro, encodeAvro, PluginInactiveError } from "../src/index.ts";
...
describe("avro stubs without the plugin", () => {
    test("every avro helper reports the inactive plugin", () => {
        expect(() => encodeAvro<Sample>({ id: 1 }, new Uint8Array(8))).toThrow(PluginInactiveError);
        expect(() => decodeAvro<Sample>(new Uint8Array(8))).toThrow(PluginInactiveError);
...
describe("avro end-to-end through the plugin", () => {
    ...
    test("avroSchema resolves to a parseable .avsc document", () => {
        ...
        expect(schema.fields.find((f: any) => f.name === "timestamp").doc).toContain("Wall-clock nanoseconds");
    });
