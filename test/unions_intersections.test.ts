...
import { generateProtobufCode } from "../src/generators/protobuf.ts";
...
describe("Unions & Intersections Coverage", () => {
    test("Key extraction for unions and intersections", () => {
        ...
        const anonInterKeys = evalModule<{ keys: string[] }>(generateKeysCode(irs.AnonInter.ir));
    ...
    test("JSON Schema generation for unions and intersections", () => {
        ...
        const namedUnionSchema = evalModule<{ schema_draft2020: any }>(generateSchemaCode(irs.NamedUnion.ir));
        expect(namedUnionSchema.schema_draft2020.enum).toEqual(["active", "inactive", "pending"]);

        // Disparate union -> anyOf
        const dispSchema = evalModule<{ schema_draft2020: any }>(generateSchemaCode(irs.DisparateUnion.ir));
        expect(dispSchema.schema_draft2020.anyOf).toBeDefined();
        expect(dispSchema.schema_draft2020.anyOf.length).toBe(2);

        // Intersections -> allOf
        const interSchema = evalModule<{ schema_draft2020: any }>(generateSchemaCode(irs.NamedInter.ir));
        expect(interSchema.schema_draft2020.allOf).toBeDefined();
    });

    test("Validator generation for unions and intersections", () => {
        const irs = getIRsForSource(sourceCode, ["NamedUnion", "AnonUnion", "DisparateUnion", "NamedInter"]);
    ...
    test("OpenAPI 3.0 and 3.1 schema generation for unions and intersections", () => {
        const irs = getIRsForSource(sourceCode, ["DisparateUnion", "NamedInter", "User"]);
