...
describe("proto ingestion agrees with protobufjs", () => {
    ...
    test("every rpc matches, streaming flags included", () => {
        ...
        for (const method of ours) {
            ...
            expect(method.request.message.name).toBe(mirror.resolvedRequestType!.fullName.replace(/^\./, ""));
            expect(method.responses[0]!.message.name).toBe(mirror.resolvedResponseType!.fullName.replace(/^\./, ""));
...
describe("emitted gRPC client", () => {
    ...
    test("all four streaming directions are emitted with the shape callers expect", () => {
        ...
        expect(source).toContain("getPet(request: GetPetOptions, options?: GrpcCallOptions): Promise<GetPetResult>;");
        ...
        expect(source).toContain("watchPets(request: WatchPetsOptions, options?: GrpcCallOptions): WatchPetsResult;");
        ...
        expect(source).toContain("upload(requests: UploadOptions, options?: GrpcCallOptions): Promise<UploadResult>;");
    ...
    const load = async (respond: (sent: Sent) => Response, sent: Sent[]): Promise<ClientModule> => {
        ...
        client.configure({
            ...
            interceptors: {
                grpc: [(call, next) => next({ ...call, headers: { ...call.headers, authorization: "Bearer token" } })],
            },
        });
        return client;
    };
    ...
    test("a gRPC interceptor can retry a failed unary call", async () => {
        ...
        const client = await load(() => new Response(trailer(), { status: 200, headers: webHeaders }), []);
        ...
        const retrying = client.createClient({
            ...
            transport: async (url, init) => {
                ...
                return sent.length === 1
                    ...
                    : new Response(concat(frame(petType.encode({ id: "p1", name: "Rex" }).finish()), trailer()), {
                          status: 200,
                          headers: webHeaders,
                      });
            },
