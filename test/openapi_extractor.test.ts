...
describe.each(['3.0', '3.1'] as const)(
  ...
  (version) => {
    ...
    test('info becomes the service identity', () => {
      ...
      expect(ir.service.methods.filter(isHttpMethod).map((m) => m.address.method)).toEqual([
        'GET',
        'PUT',
      ]);
    });
...
describe('component names survive extraction', () => {
  ...
  test('all four registries are populated', () => {
    ...
    expect(ir.components.requestBodies.get('CreateUser')!.bodies[0]!.mimetype).toBe(
      'application/json'
    );
...
describe('diagnostics and refusals', () => {
  ...
  test('a Swagger 2.0 document is refused by version', () => {
    expect(() =>
      extractApiIR(JSON.stringify({ swagger: '2.0', info: { title: 's', version: '1' } }))
    ).toThrow("[wiz] unsupported OpenAPI version '2.0'; 3.0 and 3.1 are supported");
  });
