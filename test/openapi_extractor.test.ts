...
import { extractApiIR, extractApiIRFromFile, parseApiDocument } from "../src/extractors/openapi.ts";
            ...
            schemas: {
                User: {
                    ...
                    properties: {
                        ...
                        nickname: v30 ? { type: "string", nullable: true } : { type: ["string", "null"] },
                        ...
                        kind: v30 ? { type: "string", enum: ["person"] } : { const: "person" },
                    ...
                    required: ["id", "name", "visits", "avatar", "createdAt", "tags", "role", "kind", "home", "labels"],
                },
                Address: {
                    type: "object",
...
describe.each(["3.0", "3.1"] as const)("the OpenAPI extractor inverts the generator (%s)", (version) => {
    test("a document survives extraction and regeneration", async () => {
        ...
        expect(regenerated.components.schemas).toEqual(document.components.schemas);
    ...
    test("info becomes the service identity", () => {
        ...
        expect(ir.service.methods.filter(isHttpMethod).map((m) => m.address.method)).toEqual(["GET", "PUT"]);
    });
});
...
describe("individual schema mappings", () => {
    ...
    test("a 3.0 boolean exclusiveMinimum is one bound, and round-trips", () => {
        const { ir, regenerated } = regenerate(wrap({ type: "integer", minimum: 5, exclusiveMinimum: true }, "3.0"));
    ...
    test("a 3.0 boolean exclusiveMaximum is one bound", () => {
        const { ir } = regenerate(wrap({ type: "number", maximum: 9, exclusiveMaximum: true }, "3.0"));
        expect(ir.types.get("X")!.constraints).toEqual([{ kind: "exclusiveMaximum", value: 9 }]);
    });

    test("a 3.1 numeric exclusiveMinimum passes straight through", () => {
        const { ir, regenerated } = regenerate(wrap({ type: "number", exclusiveMinimum: 5 }));
        expect(ir.types.get("X")!.constraints).toEqual([{ kind: "exclusiveMinimum", value: 5 }]);
        expect(regenerated.components.schemas.X).toEqual({
            type: "number",
            exclusiveMinimum: 5,
        });
    ...
    test("a range an integer format implies is not read back as a constraint", () => {
        ...
        const { ir, regenerated } = regenerate(wrap({ type: "integer", format: "int32" }));
        expect(ir.types.get("X")!.constraints).toEqual([{ kind: "format", value: "int32" }]);
        expect(regenerated.components.schemas.X).toEqual({
            type: "number",
            format: "int32",
            minimum: -2147483648,
    ...
    test("a bound the author wrote alongside an integer format survives", () => {
        const { ir } = regenerate(wrap({ type: "integer", format: "int32", minimum: 10 }));
    ...
    test("a mixed-type enum is a union of literals, not an enum node", () => {
        ...
        expect(x.kind === "union" && x.types.map((t) => t.kind)).toEqual(["literal", "literal"]);
    });

    test("a record keeps a string key type", () => {
        const { ir } = regenerate(wrap({ type: "object", additionalProperties: { type: "number" } }));
        expect(ir.types.get("X")).toMatchObject({
            kind: "record",
    ...
    test("the 3.0 array-of-items tuple form round-trips", () => {
        ...
        const types = [...ir.types].map(([name, typeIR]) => ({ name, ir: typeIR }));
        const code = generateOpenApiSchemaCode(types, ir.version as "3.0" | "3.1", ir.service, {
            validate: false,
        });
        const regenerated = evalModule<{
            openapiSchema: (base?: unknown) => Record<string, any>;
    ...
    test("a format that picked the primitive is not also a constraint", () => {
        const { ir } = regenerate(wrap({ type: "string", format: "int64", pattern: "^-?\\d+$" }));
        const x = ir.types.get("X")!;
...
describe("component names survive extraction", () => {
    ...
    test("all four registries are populated", () => {
        ...
        expect(ir.components.requestBodies.get("CreateUser")!.bodies[0]!.mimetype).toBe("application/json");
...
describe("path-item parameters merge with operation parameters", () => {
    ...
    test("an operation parameter replaces the path-item one of the same name", () => {
        ...
        expect(parameters.map((p) => [p.name, p.in, p.required, p.type.kind])).toEqual([
...
`;
    ...
    test("parseApiDocument sniffs each dialect", () => {
        for (const [, text] of sources) {
            expect((parseApiDocument(text) as Record<string, unknown>).openapi).toBe("3.1.0");
        }
    });
...
describe("diagnostics and refusals", () => {
    ...
    test("a discriminator mapping is reported", () => {
        ...
        expect(ir.diagnostics.map((d) => d.keyword)).toEqual(["discriminator.mapping"]);
        expect(ir.types.get("X")).toMatchObject({
            kind: "union",
            discriminator: { propertyName: "kind" },
        });
    ...
    test("an external $ref is refused by name", () => {
        expect(() => extractApiIR(wrap({ X: { $ref: "other.yaml#/x" } }))).toThrow(
    ...
    test("a $ref into a non-schema component section is refused", () => {
        expect(() => extractApiIR(wrap({ X: { $ref: "#/components/parameters/ApiKey" } }))).toThrow(
            /unsupported \$ref '#\/components\/parameters\/ApiKey'/,
        );
    });

    test("a Swagger 2.0 document is refused by version", () => {
        expect(() => extractApiIR(JSON.stringify({ swagger: "2.0", info: { title: "s", version: "1" } }))).toThrow(
            "[wiz] unsupported OpenAPI version '2.0'; 3.0 and 3.1 are supported",
        );
    });

    test("an unsupported openapi version is refused by value", () => {
        expect(() => extractApiIR(JSON.stringify({ openapi: "4.0.0", paths: {} }))).toThrow(
