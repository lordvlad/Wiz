...
beforeAll(async () => {
    ...
    const transportModule = (await import(join(directory, "transport.ts"))) as unknown as TransportModule;
...
describe("against @grpc/grpc-js over HTTP/2", () => {
    ...
    test("a deadline expires locally as DEADLINE_EXCEEDED", async () => {
        const failure = await client.never(ping("wait"), { timeoutMs: 150 }).catch((error: unknown) => error);
    ...
    test("an abort signal cancels the call", async () => {
        ...
        const pending = client.never(ping("wait"), { signal: controller.signal }).catch((error: unknown) => error);
        controller.abort();
    ...
    test("many calls share one connection", async () => {
        const answers = await Promise.all(["a", "b", "c", "d", "e"].map((text) => client.unary(ping(text))));

        expect(answers.map((pong) => pong.text)).toEqual(["pong:a", "pong:b", "pong:c", "pong:d", "pong:e"]);
    });
