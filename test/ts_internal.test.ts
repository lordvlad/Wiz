...
import { getSourceFileInternalSymbol, getSignatureSymbol, type InternalSourceFile } from "../src/tsInternal.ts";
...
describe("TypeScript internal property quarantine", () => {
    ...
    `;
        ...
        const compilerHost = {
            getSourceFile: (fileName: string) => (fileName === "test.ts" ? sourceFile : undefined),
            writeFile: () => {},
            ...
            readFile: (fileName: string) => (fileName === "test.ts" ? sourceText : undefined),
        };
