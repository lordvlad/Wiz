...
function ajvFor(draft: "2020" | "07") {
  ...
  for (const format of ANNOTATION_FORMATS) ajv.addFormat(format, true);
  return ajv;
}
...
function schemasFor(source: string, root: string) {
  ...
  const schema = evalModule<{ schema_draft2020: any; schema_draft07: any }>(
...
const corpus: Array<{ label: string; value: unknown; ok: boolean }> = [
  ...
  { label: "the optional field omitted", value: (() => { const u: any = valid(); delete u.age; return u; })(), ok: true },
  { label: "a missing required field", value: (() => { const u: any = valid(); delete u.name; return u; })(), ok: false },
  ...
  { label: "a nested constraint violated", value: { ...valid(), address: { street: "", zip: "1" } }, ok: false },
