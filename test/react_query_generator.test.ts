...
describe('Multi-tenancy & query options verification', () => {
  test('queries.ts re-uses options getter inside query hook', () => {
    ...
    expect(queries).toContain(
      'return useQuery(getListPetsQueryOptions(query, queryOptions, client));'
    );
    expect(queries).toContain(
      'return useQuery(getGetPetByPetIdQueryOptions(path, queryOptions, client));'
    );
  });
  ...
  test('mutations.ts re-uses mutation options getter inside mutation hook', () => {
    ...
    expect(mutations).toContain(
      'return useMutation(getCreatePetMutationOptions<TError>(mutationOptions, client));'
    );
    expect(mutations).toContain(
      'return useMutation(getDeletePetMutationOptions<TError>(mutationOptions, client));'
    );
  });
