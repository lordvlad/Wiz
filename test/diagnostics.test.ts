...
describe("undocumentable type diagnostics", () => {
    ...
    test("warns once per macro that has nothing to describe", () => {
        ...
        expect(
            warnings.filter((warning) => warning.includes("no methods found on object type 'Payload' for mcpSchema")),
        ).toHaveLength(1);
    });
...
describe("openapiDocument() with nothing to harvest", () => {
    ...
    test("says so rather than emitting an empty document silently", async () => {
        ...
        expect(captured.warn.some((warning) => warning.includes("found no operations reachable from"))).toBe(true);
...
describe("logger parameter", () => {
    ...
    async function withConsoleCapture(level: "warn" | "trace", run: () => Promise<void>): Promise<string[]> {
        const seen: string[] = [];
    ...
    beforeAll(async () => {
        ...
        await writeFile(fixture, UNDOCUMENTABLE.replaceAll("../../src/index.ts", join(process.cwd(), "src/index.ts")));
    });
