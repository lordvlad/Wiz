...
describe("AsyncAPI Spec Generator", () => {
    test("generates AsyncAPI 3.0 schema virtual module", () => {
        const irs = getIRsForSource(`export interface OrderEvent { orderId: string; amount: number }`, ["OrderEvent"]);
        const code = generateAsyncApiSchemaCode([{ name: "OrderEvent", ir: irs.OrderEvent!.ir }], "3.0");
    ...
    `;
        ...
        const channel = (address: string) => Object.values(doc.channels).find((c: any) => c.address === address) as any;
