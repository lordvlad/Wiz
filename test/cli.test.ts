...
describe("wiz generate through the binary", () => {
    ...
    test("a second run overwrites the outdir, and --lenient reaches the generator", async () => {
        ...
        const result = await run(["generate", "-g", "gen.ts", "api.json", "--outdir", "out", "--lenient"], cwd);
        expect(result.code).toBe(0);
  ...
  api(ir, context) {
    return { "options.json": JSON.stringify(context.options) + "\\n" };
        ...
        const result1 = await run(["generate", "-g", "gen_media.ts", "api.json", "--media-types=jsonl,yaml"], cwd);
        ...
        const result2 = await run(["generate", "-g", "gen_media.ts", "api.json", "--media-types", "all"], cwd);
  ...
  api(ir, context) {
    return { "options.json": JSON.stringify(context.options) + "\\n" };
        ...
        const result = await run(["generate", "-g", "gen_media_test_all.ts", "api.json", "--media-types=all"], cwd);
        expect(result.code).toBe(0);
