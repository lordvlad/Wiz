...
import {
  ...
  typeIdentifiers,
  typeText,
} from "../src/generators/tsTypes.ts";
...
describe("TypeIR as TypeScript", () => {
  ...
  test("a ref points at its target's identifier, and at nothing without a name", () => {
    expect(
      typeText({ id: "r", kind: "ref", targetId: "o", name: "Pet" }, new Map([["Pet", "Pet"]]))
    ).toBe("Pet");
...
describe("declaration identifiers", () => {
  test("document names are sanitised, deduped and kept clear of keywords", () => {
    const identifiers = typeIdentifiers([
      "Pet",
      "error-response",
      "error.response",
      "2xx",
    ...
    const types: Array<readonly [string, TypeIR]> = [
      [
        "Pet",
        {
          ...
          name: "Pet",
          properties: [
            { name: "id", type: primitive("string"), optional: false, readonly: false },
          ],
        },
      ],
      [
