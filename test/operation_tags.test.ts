...
describe("a type argument that documents nothing is reported", () => {
  ...
  test(
    ...
    () => {
      ...
      const matched = warnings.filter((w) =>
        w.includes("no HTTP tag found on 'Untagged' for openapiSchema")
      );
  ...
  test(
    ...
    () => {
      ...
      const matched = warnings.filter((w) =>
        w.includes("no @rpc tag found on 'Untagged' for openRPCSchema")
      );
      expect(matched).toHaveLength(1);
    },
    TIMEOUT
  ...
  test(
    ...
    () => {
      ...
      const matched = warnings.filter((w) =>
        w.includes("no @grpc tag found on 'Untagged' for grpcSchema")
      );
      expect(matched).toHaveLength(1);
    },
    TIMEOUT
