...
describe('Java Generator (models, Jakarta client, MicroProfile client, package and service overrides)', () => {
  ...
  test('honors service.package fallback and explicit package overrides (modelPackage, clientPackage, serviceName, clientName)', () => {
    ...
    expect(customClientFiles['AcmeApi.java']).toContain(
      'public class AcmeApi implements java.lang.AutoCloseable'
    );
  });
  ...
  test('generates Java Record by default with Jackson and Validation annotations on by default', () => {
    ...
    expect(userSource).toContain(
      '@JsonProperty("id") @jakarta.validation.constraints.NotNull String id'
    );
    expect(userSource).toContain(
      '@JsonProperty("age") @jakarta.validation.constraints.Min(18) @jakarta.validation.constraints.Max(120) Double age'
    );
  ...
  describe('toJavaType', () => {
    ...
    test('maps primitive types correctly', () => {
      expect(toJavaType({ id: '1', kind: 'primitive', type: 'string' }, typeNameMap)).toBe(
        'String'
      );
      expect(toJavaType({ id: '2', kind: 'primitive', type: 'number' }, typeNameMap)).toBe(
        'Double'
      );
      expect(toJavaType({ id: '3', kind: 'primitive', type: 'boolean' }, typeNameMap)).toBe(
        'Boolean'
      );
      expect(toJavaType({ id: '4', kind: 'primitive', type: 'bigint' }, typeNameMap)).toBe('Long');
      expect(toJavaType({ id: '5', kind: 'primitive', type: 'date' }, typeNameMap)).toBe(
        'java.time.OffsetDateTime'
      );
      expect(toJavaType({ id: '6', kind: 'primitive', type: 'null' }, typeNameMap)).toBe('Object');
      expect(toJavaType({ id: '7', kind: 'primitive', type: 'undefined' }, typeNameMap)).toBe(
        'Void'
      );
      expect(toJavaType({ id: '8', kind: 'primitive', type: 'void' }, typeNameMap)).toBe('Void');
      expect(toJavaType({ id: '9', kind: 'primitive', type: 'unknown' }, typeNameMap)).toBe(
        'Object'
      );
      expect(toJavaType({ id: '10', kind: 'primitive', type: 'any' }, typeNameMap)).toBe('Object');
      expect(toJavaType({ id: '11', kind: 'primitive', type: 'never' }, typeNameMap)).toBe(
        'Object'
      );
    });
  });
