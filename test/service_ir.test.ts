...
describe("capabilities the flat operation IR could not express", () => {
    test("several status codes on one method", () => {
        ...
        const doc = evalModule<{ openapiSchema: () => any }>(generateOpenApiSchemaCode([], "3.1", svc)).openapiSchema();
        ...
        expect(Object.keys(doc.components.schemas).sort()).toEqual(["NotFound", "User", "ValidationError"]);
    });
...
describe("service method key coverage", () => {
    ...
    test("request and response detail is part of the key", () => {
        ...
        expect(key({ responses: [{ ...base.responses[0]!, description: "the users" }] })).not.toBe(bare);
    });
