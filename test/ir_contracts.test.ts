...
import { computeTypeIRHash, normalizeTypeIR, walkTypeIR, type PropertyIR, type TypeIR } from "../src/types.ts";
...
describe("canonical key coverage", () => {
    ...
    test("root annotations separate two otherwise equal types", () => {
        ...
        expect(computeTypeIRHash(object({ deprecated: { isDeprecated: true } }))).not.toBe(bare);
        expect(computeTypeIRHash(object({ constraints: [{ kind: "minLength", value: 2 }] }))).not.toBe(bare);
    ...
    test("a protobuf field number separates two otherwise equal types", () => {
        expect(computeTypeIRHash(object({ properties: [property({ fieldNumber: 1 })] }))).not.toBe(
        ...
        const members: TypeIR[] = [
            {
                id: "u_1",
                kind: "object",
                properties: [property({ name: "kind", type: { id: "l_1", kind: "literal", value: "a" } })],
            },
            {
                id: "u_2",
                kind: "object",
                properties: [property({ name: "kind", type: { id: "l_2", kind: "literal", value: "b" } })],
            },
        ];
    ...
    test("a nested name reaches the payload key", () => {
        ...
        expect(normalizeTypeIR(wrap("User"), true)).not.toEqual(normalizeTypeIR(wrap("Admin"), true));
    });
});
