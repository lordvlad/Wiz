...
describe("jsonSchema and jsonSchemas macros", () => {
  test("jsonSchema generates draft 2020-12 and draft-07 single schemas", () => {
    const mod = compile(`
  ...
  test("jsonSchemas generates metaschema document with $defs for draft 2020-12", () => {
    const mod = compile(`
  ...
  test("jsonSchemas generates metaschema document with definitions for draft-07", () => {
    const mod = compile(`
      import { jsonSchemas } from "../../src/index.ts";
      export interface User {
        id: string;
  ...
  test("jsonSchemas collects transitive referenced types into $defs", () => {
    const mod = compile(`
      import { jsonSchemas } from "../../src/index.ts";
