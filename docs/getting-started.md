...
produces a stack trace pointing at the callsite. `NumberedUnion` is the one
exception, and only because it is type-level: it has no runtime existence to
begin with.
...
- [openapi](./openapi.md) — service-interface operations, `openapiSchema`,
  `openapiDocument`, and why a router is never part of the document.
