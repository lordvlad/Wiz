...
- **[AsyncAPI & Events](./asyncapi.md)** — `asyncapiSchema`, `@producer`/`@consumer` channel declarations, 2.6 and 3.0 documents on the input side, and model/codec generation.
...
- **[Java Code Generator](./java.md)** — `wiz generate -g java`, Java 17+ Records / POJOs, Jackson, Jakarta validation constraints, Lombok annotations, Jakarta REST client (`jakarta.ws.rs.client`), and MicroProfile REST client (`@RegisterRestClient` / `@RestClient`).
...
- **[CBOR Codec](./cbor.md)** — `encodeCbor` and `decodeCbor` for RFC 8949 binary, plus `application/cbor` client bodies.
...
- **[Front-End Extractors](./extractors.md)** — TypeScript, OpenAPI, AsyncAPI, OpenRPC, JSON Schema, and Protobuf front-end extractors, and `ApiDiagnostic` arrays.
